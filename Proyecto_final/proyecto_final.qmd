...
```{r}
equipo_ <- model(train, 
      equipo_ = ARIMA(gdp ~ imports + gov_exp + consumption + usd + exports + unemployment + uncertainty+ fourier(K = 1) + pdq(1,0,2) + PDQ(0,0,0))
      )
save(equipo_, file = "equipo_.RData")
```

### Mejor pronóstico

```{r}
fc_final <- forecast(equipo_, futuro)
fc_final %>% autoplot(filter_index(gdp, "2010"~.))
```
