---
title: "Predicción del crecimiento del PIB real de México"
...
```{r}
(pronostico <- model(train, 
      pronostico = ARIMA(gdp ~ imports + gov_exp + consumption + usd + exports + unemployment + uncertainty+ fourier(K = 1) + pdq(1,0,2) + PDQ(0,0,0))
      ) |> forecast(futuro))
save(pronostico, file = "pronostico.RData")
```

### Mejor pronóstico

```{r}
pronostico %>% autoplot(filter_index(gdp, "2010"~.))
```
